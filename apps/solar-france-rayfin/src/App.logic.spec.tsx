...
describe("anomalyScore", () => {
    it("is zero for nominal module temperature and inverter load", () => {
        expect(anomalyScore(plant(55, 70))).toBe(0);
    });

    it("is clamped to 1 at extreme readings", () => {
        expect(anomalyScore(plant(120, 100))).toBe(1);
    });

    it("rises as readings approach thresholds", () => {
        expect(anomalyScore(plant(75, 70))).toBeGreaterThan(anomalyScore(plant(65, 70)));
    });
});
...
describe("classifyAskIntent", () => {
    it("routes operational triage prompts to fastpath", () => {
        expect(classifyAskIntent("Any plants in alarm right now?")).toBe("ops-fastpath");
        expect(classifyAskIntent("What is the hottest module temperature?")).toBe("ops-fastpath");
    });
...
describe("signalState", () => {
    it("bands module temperature into healthy/warning/alarm", () => {
        expect(signalState("moduleTemp", 50)).toBe("healthy");
        expect(signalState("moduleTemp", 70)).toBe("warning");
        expect(signalState("moduleTemp", 85)).toBe("alarm");
    });

    it("bands inverter load into healthy/warning/alarm", () => {
    ...
    it("treats power and irradiance as informational (always healthy)", () => {
...
describe("derivePlantStatus", () => {
    it("is healthy when module temp and inverter load are nominal", () => {
        expect(derivePlantStatus(50, 70)).toBe("healthy");
    });

    it("escalates to the worst band across health signals", () => {
        expect(derivePlantStatus(70, 70)).toBe("warning");
        expect(derivePlantStatus(85, 70)).toBe("alarm");
        expect(derivePlantStatus(50, 99)).toBe("alarm");
    });
});
...
describe("ontology-driven threshold overrides", () => {
    ...
    it("adopts a valid backend band and drives classification at runtime", () => {
        expect(signalState("moduleTemp", 55)).toBe("healthy");
        const applied = applyThresholdOverrides([
            { signalKey: "moduleTemp", ontologyProperty: "ModuleTempC", unit: "\u00b0C", warn: 40, alarm: 50, governsHealth: true },
        ]);
        expect(applied).toBe(1);
        expect(signalState("moduleTemp", 55)).toBe("alarm");
        expect(signalState("moduleTemp", 45)).toBe("warning");
    });

    it("surfaces overridden bands through thresholdRows and derived status", () => {
        applyThresholdOverrides([
            { signalKey: "inverterLoad", ontologyProperty: "InverterLoadPct", unit: "%", warn: 70, alarm: 80, governsHealth: true },
        ]);
        const load = thresholdRows().find((r) => r.signalKey === "inverterLoad");
        expect(load).toMatchObject({ warn: 70, alarm: 80 });
        expect(derivePlantStatus(50, 82)).toBe("alarm");
    });
    ...
    it("ignores malformed rows (unknown key, inverted or negative bands)", () => {
        const applied = applyThresholdOverrides([
            ...
            { signalKey: "moduleTemp", ontologyProperty: "ModuleTempC", unit: "\u00b0C", warn: 90, alarm: 70, governsHealth: true },
            { signalKey: "inverterLoad", ontologyProperty: "InverterLoadPct", unit: "%", warn: -1, alarm: 50, governsHealth: true },
        ]);
    ...
    it("clearThresholdOverrides restores the compiled-in defaults", () => {
        ...
        clearThresholdOverrides();
        expect(signalState("moduleTemp", 55)).toBe("healthy");
        expect(thresholdRows().find((r) => r.signalKey === "moduleTemp")).toMatchObject({ warn: 65, alarm: 80 });
    });
});

describe("formatBand", () => {
    it("renders the warn and alarm limits for a banded signal", () => {
        expect(formatBand({ warn: 65, alarm: 80, unit: "\u00b0C" })).toBe("warn \u2265 65\u00b0C \u00b7 alarm \u2265 80\u00b0C");
    });
