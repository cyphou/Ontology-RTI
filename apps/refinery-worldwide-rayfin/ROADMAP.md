...
- [x] **Per‑site drill‑down dashboard** — site‑scoped KPIs and unit grid.
...
- [~] **Multi‑ontology / domain switch** — reuse the shell for the other accelerator domains (Wind, Solar, Manufacturing, etc.). Scaffolder `apps/tools/scaffold-domain-app.mjs` stamps a new domain app from a template via a tested domain manifest; per-domain scene/telemetry authored on top.
- [x] **Scenario / what‑if simulator** — model turndown and maintenance windows against forecast output.
